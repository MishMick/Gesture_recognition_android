...
async function cascading_images_pose_estimation(photo_path, choice) {
    ...
    fs.readdir(photo_path, async function (err, items) {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            if (path.extname(items[i]) === "" && !fs.existsSync(func_path + "keypoints.json")) {
                console.log(items[i]);
                ...
                fs.writeFileSync(func_path + "keypoints.json", JSON.stringify(pose_list));
